...
`Plan 모드`는 복잡한 기능을 구현해야 할 때, 바로 코드를 작성하는 대신 어떤 단계를 거쳐 개발할지 계획(Plan)을 먼저 제안해주는 모드입니다.   
큰 그림을 먼저 그리고, 개발자가 그 계획이 맞는지 확인한 후 코드를 생성할 수 있어 체계적인 개발이 가능합니다.  
  
채팅 창 상단 드롭다운 메뉴에서 Plan을 선택합니다.  

이번에는 단순히 하나의 파일이 아니라, 여러 단계가 필요한 작업을 요청해보겠습니다. 채팅창에 다음과 같이 입력합니다.  
```java
Todo 리스트를 관리하는 기능을 만들고 싶어요.
Todo 클래스, 저장소, 그리고 메인에서 실행하는 로직까지 단계별로 계획을 세워서 구현해 주세요.
```
  
입력을 완료하면 Plan 모드는 바로 코드를 짜는 것이 아니라, 어떻게 구현할지 단계별 목록(Step-by-step plan)을 먼저 보여줍니다.  



`Edit`에서는 선택한 Main.java 코드에서 필요한 부분만 수정했을 것입니다.  
반면, `Agent`도 작업 공간의 코드를 변경했습니다. 다만, 사용자가 따로 파일을 지정하지 않아도 스스로 필요한 파일을 찾아 여러 파일을 넘나들며 수정한다는 점에서 차이가 있습니다.  `Plan 모드`는 개발자가 "어떤 순서로 개발해야 하지?"라고 고민할 시간을 줄여주고, 복잡한 요구사항을 놓치지 않고 단계별로 처리하도록 도와줍니다.

<br>

채팅 뷰의 4가지 모드에 대해서 살펴 보았습니다.  
